...
// ---------------------------------------------------------------------------
// Shared S3 client instance hook
// ---------------------------------------------------------------------------
function getS3Client() {
    if (!process.env.NEXT_PUBLIC_AWS_REGION || !process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
        throw new Error("Missing AWS configuration. Cannot initialize S3Client.");
    }
    return new S3Client({
...
const getBucket = () => process.env.NEXT_PUBLIC_AWS_S3_BUCKET_NAME!;
...
export async function getPutPresignedUrl(
    ...
    // 🚨 We MUST force AWS to lock the exact Content-Type into the signature
